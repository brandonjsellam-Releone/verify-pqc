...
This is a **synthetic** target. The scanner is a two-layer **lexical + dependency-manifest** tool — findings are **leads to verify**, not a guaranteed-complete inventory (no full AST/data-flow, live-TLS, runtime, or binary discovery). The underlying SDK is an **unaudited reference implementation** — not a FIPS-140-3 validation or a certification. The Evidence Pack is an assurance artifact (tamper-evident given a pinned key), not a compliance attestation. See [`../../LIMITATIONS.md`](../../LIMITATIONS.md).
