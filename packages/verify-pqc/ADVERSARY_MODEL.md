...
| Over-broad tool call via an extra/unconstrained arg | Refused (least-privilege) | pqcap: arg caveats + deny_unlisted/args_allowed whitelist; strict-number arg_max; non-object args fail-closed (no array-coercion bypass of deny_unlisted); atomic consume() max_uses (no TOCTOU); declared expiry refused without a clock |
...
| Flash a swapped or rolled-back firmware | Refused before flash | pqfirmware: manifest binds the binary's hash + version strictly-newer (anti-rollback); device verify-before-flash; anti-rollback is FAIL-CLOSED — a valid-integer currentVersion is required (no silent skip / NaN bypass); requireModel binds the hardware |
...
| Replay a payment authorization (double-spend) | Refused (fail-closed) | pqpay: nonce normalized to string + verifyAuthorization fail-closed without a durable seen-nonce ledger; verifyAndConsume prefers an ATOMIC consume() test-and-set (no has/add TOCTOU); a declared expires_at is refused without a clock (no silent expiry bypass) |
