...
| Tampered ciphertext / proof / signature | Rejected (fail-closed) | AEAD auth tags; signature verify; fuzz sweep = 0 fail-open OBSERVED across 4,176 inputs (verifiers made total) |
| Malformed/adversarial input to a verifier | No crash (fail-closed) | `fuzz-robustness.mjs`: 4,176 calls (58 verifiers × 72 input classes), no throw observed; `tamper-binding.mjs` 781-field coverage |
