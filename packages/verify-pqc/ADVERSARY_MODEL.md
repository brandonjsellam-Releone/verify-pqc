...
| Tampered ciphertext / proof / signature | Rejected (fail-closed) | AEAD auth tags; signature verify; fuzz sweep = 0 fail-open OBSERVED across 4,248 inputs (verifiers made total) |
| Malformed/adversarial input to a verifier | No crash (fail-closed) | `fuzz-robustness.mjs`: 4,248 calls (59 verifiers × 72 input classes), no throw observed; `tamper-binding.mjs` 1,011-assertion field-binding coverage |
